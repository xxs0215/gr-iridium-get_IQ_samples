...
void iridium_frame_printer_impl::handler(const pmt::pmt_t& msg)
{
    ...
    pmt::pmt_t burst_rrc = pmt::dict_ref(meta,pmt::mp("burst_rrc"),pmt::PMT_NIL);//添加burst_rrc数据
    ...
    //写入rrc数据
    if(pmt::is_c32vector(burst_rrc)){
        size_t len;
        const gr_complex *rrc_data = pmt::c32vector_elements(burst_rrc, len);
        std::string filename = "/home/xxxxs/signals/burst_rrc_" + std::to_string(id) + ".cfile";
        FILE *fp = fopen(filename.c_str(), "wb");
        fwrite(rrc_data, sizeof(gr_complex), len, fp);
        fclose(fp);
    }
